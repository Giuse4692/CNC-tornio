...
"""
    ...
    def prepare_simulation(self):
        ...
        self.gcode_listbox = tk.Listbox(self.left_frame)
        self.gcode_listbox.pack(padx=10, pady=10, fill="both", expand=True)
        for instruction in gcode_instructions:
            self.gcode_listbox.insert(tk.END, instruction.strip())

        self.start_simulation_button = ttk.Button(self.left_frame, text="Avvia Simulazione", command=lambda: self.simulate_program(gcode_instructions))
        self.start_simulation_button.pack(pady=10)

        self.back_button = ttk.Button(self.left_frame, text="Indietro", command=self.cancel_new_program)
        self.back_button.pack(pady=10)

        self.show_message("Premi 'Avvia Simulazione' per iniziare.")
            def simulate_program(self, gcode_instructions):
        """Simula le istruzioni G-code sul grafico."""
        self.show_graph()
        self.ax.clear()
        self.ax.set_xlabel("X")
        self.ax.set_ylabel("Y")
        ...
        x, y = 0, 0
        for idx, instruction in enumerate(gcode_instructions):
            instruction = instruction.strip()
            self.gcode_listbox.itemconfig(idx, {'bg':'yellow'})  # Evidenzia l'istruzione corrente
            self.canvas.draw()
            self.root.update()
            time.sleep(1)
            self.gcode_listbox.itemconfig(idx, {'bg':'white'})  # Rimuove l'evidenziazione
            ...
            if instruction.startswith('G1'):
                ...
                self.ax.plot(x, y, 'bo')
                self.canvas.draw()
                self.show_message(f"Eseguendo: {instruction}")
            elif instruction.startswith('G0'):
                parts = instruction.split()
                for part in parts:
                    if part.startswith('X'):
                        x = float(part[1:])
        ...
        self.show_message("Simulazione completata")
